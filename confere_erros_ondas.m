function [matriz_out num_rep num_gap num_sinc num_amb_T ,...
    num_amb_Dir num_amb_H dados_excluidos dados_eliminados percent_good,...
    tempo]=confere_erros_ondas(matriz_in,time,corte_in,corte_fin,eO,nome_est)
...
count=0;count2=0;
    if corte_in ==0 && corte_fin==0
        matriz_out(eO)=nan;
        count=count+1;
    end
for i=1:length(eO)
    if eO(i)-corte_in>=1 && eO(i)<length(matriz_out)
...
%encontra erros ambientais de altura Hmean
altura=matriz_out(:,4);
...
dados_eliminados=num_gap+num_rep+num_sinc+count+count2+num_amb_T+num_amb_Dir+num_amb_H;
