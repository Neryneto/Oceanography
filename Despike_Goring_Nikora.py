...
def PhaseSpaceDespike1d(vel):
    ...
    while (n_out > 0 & numberLoop <= numberMaxIteration):
        ...
        if numberLoop == 1:
            theta = np.arctan2(np.nansum(vel * secondDerivative),
                               np.nansum([i**2 for i in vel]))

        # Step 3 - checking outlier in the 3D phase space
        excludedXx, excludedXy, excludedXz, m, ind, coef = ExcludeOutlierEllipsoid3D(
            vel, firstDerivative, secondDerivative, theta, universalThreshold)
